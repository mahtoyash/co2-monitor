import { db, ref, onValue, push, set, get } from "./firebase.js";
...
function scaleFeatures(row) {
  // row = [temperature, humidity, co2, occupancy, hour, day_of_week, minute]
...
// ── Load base model from GitHub ───────────────────
async function loadBaseModel() {
  console.log("Loading base model...");
  baseModel = await tf.loadLayersModel(MODEL_URL);
  console.log("Base model loaded!");
}

// ── Load per-room weights from Firebase ──────────
async function loadRoomModel(roomId) {
  const weightsRef = ref(db, `model_weights/${roomId}`);
  const snapshot = await get(weightsRef);

  if (snapshot.exists()) {
    console.log(`Room weights found for ${roomId}, loading...`);
    ...
    console.log("No saved weights, using base model...");
    roomModel = await tf.loadLayersModel(MODEL_URL);
  }
}

// ── Save per-room weights to Firebase ────────────
...
async function runPrediction(history) {
  ...
  const input = tf.tensor3d([scaled]); // [1, 30, 7]
...
async function onlineTrain(history) {
  ...
  const targetScaled = [
    (target.co2 - TARGET_MIN) / (TARGET_MAX - TARGET_MIN),
    (target.co2 - TARGET_MIN) / (TARGET_MAX - TARGET_MIN),
    (target.co2 - TARGET_MIN) / (TARGET_MAX - TARGET_MIN)
  ];

  const xs = tf.tensor3d([scaled]);
  const ys = tf.tensor2d([targetScaled]);
  ...
  // Har 10 readings ke baad weights save karo Firebase mein
...
function listenToRoom(roomId) {
  ...
  onValue(histRef, async (snapshot) => {
    co2History = [];
...
// ── Occupancy update ──────────────────────────────
window.updateOccupancy = function () {
